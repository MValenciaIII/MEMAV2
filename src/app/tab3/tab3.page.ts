...
export class Tab3Page implements AfterViewInit {
  ...
  async ngAfterViewInit() {
    await this.storage.get('alertData');
  }
  ...
  private async checkForWeatherAlerts(done=null) {
    let citiesWithAreas = Object.values(this.cityPoints).filter(point => (point.alertArea));
    let NWSQueries: Promise<null>[] = []; // only accept Promise type
    citiesWithAreas.forEach(city => {
  ...
  private async getAlertAreasFromStorage() {
  ...
  private async saveAlertAreasToStorage() {
    let data = {};
    Object.values(this.cityPoints).filter(point => (point.alertArea)).forEach(point => {
